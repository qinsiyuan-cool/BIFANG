...
namespace antlr4 {
  ...
  public:
    ...
    ///成员变量，分别表示与当前规则上下文相关联的第一个和最后一个标记（Token）。
    /// 这些标记用于确定规则匹配的文本范围。
    Token *start;
    Token *stop;
    ...
    ///这些方法用于获取与特定标记类型相关联的标记节点。getToken 返回指定类型和位置的单个标记节点，
    /// 而 getTokens 返回所有指定类型的标记节点的向量。
